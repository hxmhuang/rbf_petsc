...
!   !do j=0,0
 		if(myrank==0) print *,">Rbfmatrix_df_hypre: current", j, " step"
  		...
  		ximat  	= dm_submatrix(x,imat,dm_m2n(0,2)) 
    	...
    	B=B .vj. (dm_zeros(1,1))
    	weights= A .inv. B
    	!call dm_view(weights,ierr)
    	weightsDx= weightsDx .vj. (dm_submatrix(weights, dm_m2n(0,fdsize-1), dm_zeros(1,1))) 
 		!call dm_view(weightsDx,ierr)
    	
     	B=(-1.0)*ximat * dm_trans(dm_getrow(atm%pts%p_v,j)) .em. drbf_rd2v
     	B=B .vj. (dm_zeros(1,1))
    	weights= A .inv. B
    	weightsDy= weightsDy .vj. (dm_submatrix(weights, dm_m2n(0,fdsize-1), dm_zeros(1,1))) 
    		
    	B=(-1.0)*ximat * dm_trans(dm_getrow(atm%pts%p_w,j)) .em. drbf_rd2v
    	B=B .vj. (dm_zeros(1,1))
    	weights= A .inv. B
    	weightsDz= weightsDz .vj. (dm_submatrix(weights, dm_m2n(0,fdsize-1), dm_zeros(1,1))) 
    	...
    	B=B .vj. (dm_zeros(1,1))
    	weights= A .inv. B
    	weightsL= weightsL .vj. (dm_submatrix(weights, dm_m2n(0,fdsize-1), dm_zeros(1,1))) 

    enddo
	!call dm_view(weightsL,ierr)
